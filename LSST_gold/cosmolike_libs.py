...
update_nuisance_LSST_source = lib.update_nuisance_LSST_source
update_nuisance_LSST_source.argtypes=[]

update_Ntable_Na = lib.update_Ntable_Na
update_Ntable_Na.argtypes = [ctypes.c_int]

get_nuisance_lens_z_s = lib.get_nuisance_lens_z_s
get_nuisance_lens_z_s.argtypes = []
get_nuisance_lens_z_s.restype = ctypes.c_double

get_nuisance_source_z_s = lib.get_nuisance_source_z_s
get_nuisance_source_z_s.argtypes = []
get_nuisance_source_z_s.restype = ctypes.c_double

get_nuisance_A_ia = lib.get_nuisance_A_ia
get_nuisance_A_ia.argtypes = []
get_nuisance_A_ia.restype = ctypes.c_double

get_nuisance_beta_ia = lib.get_nuisance_beta_ia
get_nuisance_beta_ia.argtypes = []
get_nuisance_beta_ia.restype = ctypes.c_double

get_nuisance_eta_ia = lib.get_nuisance_eta_ia
get_nuisance_eta_ia.argtypes = []
get_nuisance_eta_ia.restype = ctypes.c_double

get_nuisance_Mobs_lgM0 = lib.get_nuisance_Mobs_lgM0
get_nuisance_Mobs_lgM0.argtypes = []
get_nuisance_Mobs_lgM0.restype = ctypes.c_double

get_nuisance_Mobs_alpha = lib.get_nuisance_Mobs_alpha
get_nuisance_Mobs_alpha.argtypes = []
get_nuisance_Mobs_alpha.restype = ctypes.c_double

get_nuisance_Mobs_beta = lib.get_nuisance_Mobs_beta
get_nuisance_Mobs_beta.argtypes = []
get_nuisance_Mobs_beta.restype = ctypes.c_double

get_nuisance_Mobs_sigma = lib.get_nuisance_Mobs_sigma
get_nuisance_Mobs_sigma.argtypes = []
get_nuisance_Mobs_sigma.restype = ctypes.c_double

get_nuisance_cluster_c = lib.get_nuisance_cluster_c
get_nuisance_cluster_c.argtypes = [ctypes.c_int]
get_nuisance_cluster_c.restype = ctypes.c_double

get_nuisance_gbias = lib.get_nuisance_gbias
get_nuisance_gbias.argtypes = [ctypes.c_int]
get_nuisance_gbias.restype = ctypes.c_double
...
class InputNuisanceParams(IterableStruct):
    ...
    def fiducial(cls):
        ...
        c.lens_z_s = 0.01
        c.source_z_s = 0.05
        c.bias[:] = [1.389274,1.531318,1.636866,1.738404,1.843594,1.960837,2.097591,2.269669,2.523241,3.481663]
        c.A_ia = 5.92
        c.beta_ia = 1.1
        c.eta_ia = -0.47
    ...
    def fiducial_sigma(cls):
        ...
        c.lens_z_s = 0.0005
        c.source_z_s = 0.001
        c.bias[:] = np.repeat(0.03, 10)
        c.shear_m[:] = np.repeat(0.001, 10)
    ...
    def setup(cls):
        c = cls()
        c.lens_z_s = get_nuisance_lens_z_s()
        c.source_z_s = get_nuisance_source_z_s()
        c.bias[:] = [get_nuisance_gbias(0), get_nuisance_gbias(1), get_nuisance_gbias(2), get_nuisance_gbias(3), get_nuisance_gbias(4), get_nuisance_gbias(5), get_nuisance_gbias(6), get_nuisance_gbias(7), get_nuisance_gbias(8), get_nuisance_gbias(9)]
        c.A_ia = 5.92 # get_nuisance_A_ia()
        c.beta_ia = 1.1 # get_nuisance_beta_ia()
        c.eta_ia = -0.47 # get_nuisance_eta_ia()
        c.m_lambda[:] = [get_nuisance_Mobs_lgM0(), get_nuisance_Mobs_alpha(), get_nuisance_Mobs_beta(), get_nuisance_Mobs_sigma()]
        c.cluster_c[:] = [get_nuisance_cluster_c(0), get_nuisance_cluster_c(1), get_nuisance_cluster_c(2), get_nuisance_cluster_c(3)]
        return c
...
def sample_cosmology_pos_pos_nuisance(tomo_N_lens,MG = False):
    varied_parameters = sample_cosmology_only(MG)
...
def sample_cosmology_pos_pos_nuisance_specz(tomo_N_lens,MG = False):
    varied_parameters = sample_cosmology_only(MG)
    varied_parameters += ['bias_%d'%i for i in xrange(tomo_N_lens)]
    return varied_parameters
...
def sample_cosmology_ggl_clustering_nuisance_specz(tomo_N_shear,tomo_N_lens,MG = False):
    varied_parameters = sample_cosmology_only(MG)
    varied_parameters += ['bias_%d'%i for i in xrange(tomo_N_lens)]
    varied_parameters += ['source_z_bias_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters.append('source_z_s')
...
def sample_cosmology_2pt_nuisance_specz(tomo_N_shear,tomo_N_lens,MG = False):
    varied_parameters = sample_cosmology_only(MG)
    varied_parameters += ['bias_%d'%i for i in xrange(tomo_N_lens)]
    varied_parameters += ['source_z_bias_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters.append('source_z_s')
...
def sample_cosmology_2pt_nuisance_IA_marg_specz(tomo_N_shear,tomo_N_lens,MG = False):
    varied_parameters = sample_cosmology_only(MG)
    varied_parameters += ['bias_%d'%i for i in xrange(tomo_N_lens)]
    varied_parameters += ['source_z_bias_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters.append('source_z_s')
...
def sample_main(varied_parameters, iterations, nwalker, nthreads, filename, blind=False):
    ...
    starting_point += InputNuisanceParams().setup().convert_to_vector_filter(varied_parameters)
    ...
    f = open(filename, 'w')
    f_like = open(filename+"_loglike", 'w')
